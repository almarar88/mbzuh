...
export function getClient(): Anthropic | null {
  ...
  // dangerouslyAllowBrowser: على أندرويد يعمل الطلب من WebView مباشرة (المفتاح محلي على جهاز المستخدم)
  const client = new Anthropic({ apiKey: key, maxRetries: 2, timeout: 10 * 60 * 1000, dangerouslyAllowBrowser: true });
  cached = { key, client };
  return client;
}
